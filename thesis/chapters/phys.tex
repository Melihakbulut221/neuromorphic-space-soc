...
ROM's error-correcting code (\dcite{67}{section 11}), and the only
layout in the project that meets hold at all three corners. Its own
...
2026-08-25 in this environment). The PDK commit is not chosen: every
LibreLane release ships \code{pdk\_hashes.yaml} naming the commit it was
tested against, and the runner reads that pin out of the installed wheel
...
\code{keep\_hierarchy} attribute the triple-modular register bank
carries as an anti-merge defence and \code{Checker.\allowbreak
YosysUnmappedCells} stops the run; under \code{deferred\_flatten}, a
constant driving the core's \code{cheriot\_enable\_i} never folds, and
the same RTL comes out as 37,895 cells of
\qty{523809.6444}{\micro\metre\squared} against 27,565 of
...
the core --- the core less the macros --- is
\qty{2001390}{\micro\metre\squared}, of which the standard cells fill
\qty{46.6}{\percent}. The first layout of this SoC put
\qty{396177.6420}{\micro\metre\squared} of cells into
\qty{2092010.95}{\micro\metre\squared}, \qty{18.9}{\percent}, and
...
\begin{figure}[tbp]
  ...
  pocket'' rectangle comes from \code{clint\_region.py}'s constants,
  which describe the six-macro floorplan in which the lower ROM sat at
  $y=60.48$; here that band holds the check macro.}
  \label{fig:phys-density}
\end{figure}

\Cref{fig:phys-density} is the same placement as a density field, and it
shows the property the timing story turns on: \emph{the channel is not
where all the logic is}. The zone census on the six-macro floorplan puts
...
and strip 813 more. A net with one pin in a pocket and the other in the
channel crosses a ROM macro, and 140 nets did so in one run and 134 in
the other (\dcite{72}{section 6.4}).
...
max-fanout
checker at all, on a layout carrying 589 such violations against a
constraint of 10 and a library declaring \code{default\_max\_fanout : 8}.
...
\qty{-13195.08}{\nano\second} of total negative slack; the others are 360 endpoints from a RAM macro's \code{A\_DOUT[31]}, 154 from
...
single-error-correcting double-error-detecting code. It decomposes
into \qty{2.492291}{\nano\second} of launch clock --- through the core's
integrated clock gate, which is in the launch path --- and \textbf{77
driver stages carrying \qty{26.290}{\nano\second} of cell delay},
arriving at \qty{29.565346}{\nano\second} against a required
...
or inverters the repair steps inserted.
...
\qty{5.2678}{\nano\second} at 256 and 512 words to
\qty{9.6611}{\nano\second} at 8192 while density improves in the same
direction, a \qty{4.3933}{\nano\second} trade end to end. The part
...
guessed at}: the step was re-executed on its own saved state with
OpenROAD's \code{repair\_setup} trace on, reproducing the flow's step to
the byte --- 53 metrics, zero differing --- and printing why the loop
stopped. No configured limit bound; what ended it was OpenROAD's own
rule abandoning \code{repair\_timing} when total negative slack has not
moved by \qty{0.01}{\percent} for two hundred iterations. It chased its
worst endpoint for 257 iterations and improved it, tried nineteen more
for 51 passes each and reverted every change, and quit with
\textbf{3,345 of 3,365 endpoints never visited}. The stage that defeated
it drove \qty{180}{\femto\farad} from the channel at
$(1911.84,\,1224.72)$ into the pocket at $(1973.76,\,1776.60)$ ---
\qty{614}{\micro\metre} of wire over a \qty{336}{\micro\metre} macro at
a \qty{4.69}{\nano\second} transition, \qty{3.73}{\nano\second} on one
single-drive-strength gate (\dcite{72}{sections 4 to 7}).
...
(\cref{sec:phys-rdreg} is the second). The first is
the antenna repair. \code{s83romecc5} leaves two antenna violations ---
...
worst path} as the metric rather than the slack alone.
...
route}: the channel has no \qty{38000}{\micro\metre\squared} to give,
3,076 cells spread into the macro gaps, and the global router ends with
420 units of overflow. The flow's own manual-placement key, which moves
...
macros. On the corrected tool the rule's answer is 999 cells, and the
block's register-to-register closure is 1,830
...
\qty{-28.87}{\nano\second}; sixteen of the seventeen survivors are not
on the bus at all but capture into the scrub engine's counter. The
...
register-file read into the fabric, which the read register was never
...
shape this design's flow drew in open area. The total is not available,
because no open deck can check these macros at the pinned PDK version.
One check that did not run is recorded with the ones that did: the
detailed router skips an enclosure rule on nine layers, identically in
the hardened and baseline runs, so it is the PDK's and every layout has
inherited it.
...
mode on the same file returns 122,814, a factor of 53.0. Any figure
...
--- and evaluated all of them over the whole 113\,MB layout. Three
fired, all inside vendor geometry.
...
foundry precheck, and the deck that returns zero is the extra set, which
the same documentation calls not verified or tested. DRC was never the
binding constraint anyway --- a design that passes DRC and fails LVS is
not signed off (\dcite{54}{sections 3 and 7}).
...
default activity assumption and must be quoted as one}: reproducing it
exactly requires asserting that every top-level input and every register
output toggles 0.1 times per clock period and spends half its time at 1
(\dcite{57}{section 2}).

Measured instead at the toggle rates of a real program --- on an earlier
six-macro layout without the accelerator, so superseded for this design
--- the same recipe gives \qty{33.890}{\milli\watt} computing and
...
measured 2026-09-06). \textbf{Idle is \qty{16.3}{\percent} of busy, and
...
modelling stand-in, and it covers \qty{79.9}{\percent} of the design's
...
\qty{-7.5758}{\nano\second} on 3,529
endpoints at the slow corner, on the same register-file read into the
fabric that every layout has found. Four leads were measured to sign-off
and none recovers it --- the hierarchy mode and both post-global-route
stages were banked before the first number existed, two alternative
floorplans are worse and one does not route, and the capacitance
constraint costs more than it buys. The two unpriced mechanisms are a
registered request phase in the bus fabric and a hold-repair pass aimed
at the fast corner that would let the read register be turned on.
...
\textbf{The redundancy is in the netlist and not in the placement.}
Nothing in this flow requests physical separation between
triple-modular replicas and nothing supplies it by default: on the
frozen pilot's sign-off layout the median centre-to-centre distance
between corresponding bits of different replicas is
\qty{27.52}{\micro\metre} against \qty{113.47}{\micro\metre} over all
cross-replica pairs, a $4.12\times$ concentration with 49 pairs
abutting, and the same picture appears on this SoC across three
