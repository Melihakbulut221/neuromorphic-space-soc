...
consequence (\dcite{69}{section 1}). In the accelerator's connection the
...
(\code{thesis/figures/make\_layout\_figures.py}; its docstring counts
\num{168592} instances and its output file \num{168584}, and these
figures are the output file's). The remainder --- \num{5916} cells,
\SI{9.75}{\percent} of the \num{60694} non-fill cells --- is drawn grey
rather than hidden, and \num{107890} fill, decap and diode instances are
not drawn. The protection's own logic is five legend entries: RAM
...
functions --- 2, 6, 14 at $W = 1,2,3$ --- and three replicas need six,
so \textbf{three bits is the first width at which a third replica has a
function left to take} (\dcite{30}{section 3.3};
\dcite{41}{section 4.1}). Six of the watchdog's eight protected fields
...
(\dcite{41}{section 3.2}). The report fields are \emph{inside} the word,
not beside it, because \dcite{16}{section 5.8} had measured this
repository's own safety-net report being the single point of failure:
an upset could erase the announcement of the event it caused.
...
\code{ibex\_regfile\_secded.v} carries upstream's module name and port
list, and the build is given it instead of upstream's
\code{ibex\_register\_file\_ff.v}, so the Ibex checkout stays a pristine
clone at the pinned commit. The cost is recorded rather than glossed:
an upstream port whose \emph{meaning} changes without its name changing
is caught by nothing (\dcite{43}{section 3.2}).
...
\code{soc\_busstat.v}. The demonstration is a load instruction and
not a hierarchical read: all four of the core campaign's \code{x23}
records end with the golden answer and with software reading
\code{CNT\_RFSEC = 1}.

It does not protect the addresses, since a corrupted \code{raddr} or
\code{waddr} reaches the wrong register with a perfectly valid codeword,
nor the other \SI{55}{\percent} of the core, where the controller, fetch
FIFO and load/store FSM carry the highest per-bit rates in the design.
...
faulty one; truncating to 54 bits saves
\SI{1438.1766}{\micro\meter\squared} and still leaves an upset that ends
the mission --- ``not a protection; a cheaper failure''
...
same Hsiao code with 56 data bits tied off, which shortens to the
minimal SECDED width for a byte. Four of them is exactly 64 bits: no
seventh macro, no die growth, no read-modify-write, because a byte write
lands through the macro's own per-bit write mask with its check bits
...
uncorrectable syndrome answers the read with a bus error, which Ibex
takes as an access fault, so \textbf{the recovery is a trap, not a wrong
word}.
...
are those two failures (\dcite{67}{section 4.3}).
...
standard-cell area, with the die not one database unit larger, 0
detailed-routing DRC errors and hold closed at three corners. The timing
is the honest half. Pre-layout the decoder fit on the read return with
...
group's violating population from \textbf{68 endpoints at
\SI{-2.7112}{\nano\second} to \num{1080} at
\SI{-4.0021}{\nano\second}}. Its endpoint is inside the register file: a
load's data now crosses the memory decoder \emph{and} the register
file's own SECDED encoder in one cycle --- \textbf{two independent codes
over one word}, which this project would not have chosen deliberately.
It is still not the binding path, which remains the register file's read
address (\dcite{67}{section 5.3}).
...
than the flash, at 17.01 cycles per word, so a word that arrived intact
and landed on a row whose codeword did not take is caught.
...
domains one at a time on that same netlist and found the worst path
starting \emph{inside} the accelerator, where every fault line starts,
at only \SI{-1.3129}{\nano\second}. The \SI{-5.0198}{\nano\second}
launches from \code{u\_ibex.\dots raddr\_a\_i[2]}, \textbf{the
flip-flop that also gives the whole design its worst slack}. The repair
was built anyway, for a better reason: the enable is split by
\emph{arrival}, with only the two signals that can move while the clock
is stopped left combinational and every term that is a function of the
block's own registers moved behind one flip-flop on the ungated clock.
Such a term cannot pulse and vanish, so the cost is \textbf{one cycle of
wake latency on a fault and one flip-flop}, proved rather than hoped
(\dcite{77}{sections 1, 3 and 6}).
...
cost \textbf{one flip-flop} and
\SI{+643.0536}{\micro\meter\squared} on the whole SoC, with no die
area, no hold margin at any corner and no DRC; a block-level area figure
was published and then \textbf{withdrawn} on 2026-09-09 because it did
not reproduce under any of nine recipes, and it is not quoted here. The
added bit \emph{cannot} be tripled --- the one-bit bound again --- and
neither corruption direction resets the part.
...
It also costs software a rule rather than hardware area:
\code{WINS = s} requires $i_{\max}/i_{\min} < 1/(1-2^{-s})$ on the kick
cadence. The original workload, kicking wherever the code happens to
pass, measures a jitter ratio of \textbf{57.2}, which fits inside no
window at all; the supervisor written to be what the software will be
measures 1.412, admitting \code{WINS = 1} and nothing tighter, and run
work-triggered rather than time-triggered it measures 2.104 ---
\textbf{at which no timeout at all satisfies both bounds}
(\dcite{46}{sections 5 and 7}).
...
netlist it was false --- structural hashing had collapsed three declared
...
87 to 58 and 29 flip-flops become shared by two voter inputs;
\textbf{the cone census catches it and nothing else does}, and only if
...
rungs followed: a guard that runs the cone census over every whole-SoC
...
flip-flops --- and for the codec, with two negative controls whose
passing the Makefile treats as an error (\dcite{79}{section 2}).
...
single-event cross-section, charge-collection radius or LET threshold
for this process was measured and none is available; the measurement
establishes that this layout \emph{permits} a common-mode upset over
much of the protected word and says nothing about whether one occurs at
any fluence. ``Because the replicas are a few micrometres apart, one ion
track defeats the vote'' is therefore forbidden: it silently supplies a
...
nothing about any other cell, nothing about placement, nothing about
whether the replicas are \emph{correct}, and nothing about the 36 of 102
flip-flops it counts that are deliberately unprotected. The fix is never
to widen the claim; it is to build a second instrument at the level the
...
the explanation was wrong, and only looking at the paths found it.
\dcite{76} diagnosed its clock-gate failure as the fault-line cone and
\dcite{77} measured that cone at \SI{-1.3129}{\nano\second} of a
\SI{-5.0198}{\nano\second} miss whose real launch was the CPU's own
critical path. \dcite{61} estimated a cost at \SI{+4.4}{\percent} by
carrying an earlier absolute area onto a new denominator, and
\dcite{62}{section 1} measured \SI{+4.9406}{\percent} and called the
method the error rather than the arithmetic. The defence is an
...
not touch found \textbf{0 of 198 records changed} --- not a class, not a
displacement, not a drawn cycle --- and \dcite{69} found the same zero
over 296. \dcite{44} re-ran the entire core campaign and found the two
...
report reading those counters would have credited the window with a
...
is 528 cells (\dcite{58}{section 11}), and the voter has no state to
inject into at all.
