...
One more convention, inherited from \dcite{64}: a superseded measurement
...
which \dcite{34} pins by git blob hash for the TTIHP26b submission; running it inside a verification pass rewrote that file's
...
second-resolution timestamp, and \dcite{66} measured what the timestamp
...
2026-09-11 row at 509 passing, 0 failing, which is superseded by the
collection count above; \dcite{44}{section 8.3} reports 315 at its own
writing and \dcite{43} closed at 295, and both figures are left standing
as records of their date. The largest files are
\code{test\_doc\_links.py} (96 tests), \code{test\_soc\_synthesis\_guards.py}
(68), \code{test\_lif\_core.py} (41), \code{test\_synthesis\_guards.py}
(40) and \code{test\_secded.py} (37).
...
The governing discipline is \dcite{78}'s, and it was learned twice in
that document alone.
...
was run with it until \dcite{63}. That document is the bring-up, and it
...
are different files --- a division kept unchanged since \dcite{16}.}
\label{tab:verif-fi}
\small
...
\midrule
\dcite{42}{} & RTL & 1{,}300 & --- & the unhardened core, 13 strata
weighted by 2{,}198 bits \\
\dcite{43}{} A & RTL & 1{,}400 & 2{,}800 & the SECDED register file,
same draws, byte-identical workload image \\
\dcite{43}{} AW & RTL & 1{,}400 & 2{,}800 & the watchdog's cadence
contract on top of A \\
\dcite{44}{} & RTL & 1{,}400 & 2{,}800 & the fast-correction path and
the fault-report port \\
\dcite{46}{} & RTL & 812 & 1{,}624 & the window, on a partitioned
supervisor workload, 58 draws per stratum \\
\dcite{74}{} & gate & 1{,}424 & 1{,}739 & \dcite{43}'s records replayed
on the sign-off netlist \\
\dcite{75}{} & gate & 174 & 4 netlists & the reset on a corrected upset:
...
\dcite{44} re-ran that campaign on a design with a rebuilt register read
...
\dcite{46}'s campaign is the one that produced a design decision. The
...
\dcite{74} is the first gate-level simulation of the whole system in this
...
after the upset.} \dcite{75} is that finding. The watchdog's reset
