...
The neuron model is classical digital leaky integrate-and-fire. Each
...
all neurons: \dcite{02}{section 3}'s Candidate~B, chosen because
flip-flop weights do not scale.

The physical word is 64 data bits, exactly 16 weights, plus 8 check
bits of a single-error-correcting double-error-detecting code: a 72-bit
macro word at 12.5\,\% overhead. At the 512-by-512 default that is
...
not accumulate into double-bit ones.
...
macro and returned a \textsc{no-go} for this shuttle, because the macro
...
shuttle submission itself, frozen by blob hash and unmodified, is
\emph{instantiated} inside \code{soc\_top.v} and reached over the same
four serial pins the die will have. No copy exists under
\code{hw/soc/}, and a guard suite fails if any of the seven frozen
files it reads is touched.
...
program requires a load-access fault for all of them.
...
\code{E\_DECIDE} is bounded because it holds while the die's input
queue is full and only a drain unblocks it; its escape returns there
with the event word untouched, so no event is lost.
...
configurations differ in no key. The accelerator's share had been
measured before the gate existed: it multiplied the chip's idle power
by 5.588, and all 2,178 flip-flops it brought sat on the ungated net
(\dcite{57}{section 9}, \dcite{61}{section 7.3}). It is one domain
...
while the clock is stopped, and the eight fault terms that are
functions of the block's own registers move behind one flip-flop on the
ungated clock. Such a term cannot pulse and vanish --- it rises when
...
makes it evidence. \code{hw/soc/flow/\allowbreak{}gen\_npu\_\allowbreak{}vectors.py}
runs on every build of the boot image, \emph{before} the compiler,
running \code{sw/golden/lif\_core.py} over the stimulus and emitting
the node register map in C plus the stimulus and the expected output
stream. The program compares against those constants and never asks the
hardware what the answer is --- the difference between a check and a
tautology. The stimulus is the pilot's own testbench's, unchanged, so a
...
70\,\% planning criterion, and the submission moved to twelve tiles in
a six-by-two arrangement, on routing convergence, timing and clock skew
as well as area.
...
toolchain and process-kit versions.
...
software interface. The per-node counters, the node identifier and the
routing prefix are specification and reservation, and a reader should
not count them as hardware.
...
and drew 100 times into each, and the result did not match the ranking
...
\textbf{For two waves the one-line claim was checked by one simulation
test and one textual guard rather than over all reachable states.}
\dcite{55}{section 9.3} named the gap and declined to close it;
...
\code{thesis/figures/\allowbreak{}make\_layout\_figures.py} from the
sign-off run's placement database and netlist.}
\label{fig:npu-layout}
\end{figure}

\Cref{fig:npu-layout} answers ``where is the accelerator'' for the only
layout of the full system-on-chip that has one, and it must be read
with its method in mind. The die carries 168,592 instances --- the
attribution file the same script writes totals 168,584, a difference of
eight --- of which only about 7,700 still have a hierarchical name
after synthesis, so a picture that coloured only those would colour
four per cent of the die and put the processor in the wrong place. The
remainder is attributed by connectivity: nets keep the hierarchical
names Yosys did not create, so every net votes for a block; each
anonymous cell takes the block most of its pins' nets vote for; and two
rounds of neighbour propagation resolve the rest.
...
request, the queues' stored words and the frame guards --- and the
reason is a measurement rather than a budget. The queues' storage is
41.2\,\% of the connection's flip-flops and 53.5\,\% of its area and
produced zero silent wrong inferences in 100 draws, because occupancy
is low and most of those bits are storage nothing will read: a wave
that started with the biggest structure would have bought nothing. The
largest site now left is the engine's state register, at 7 of 20, which
...
gate-level netlist of the connection has ever been simulated, no
back-annotated timing, no multi-bit strike, and no single-event
transient in combinational logic --- which matters because the strobe
gate \emph{is} combinational logic, so a transient on it is a failure
mode no instrument here can see.
