...
\paragraph{Scale.} From six macros to eight, markers rise by \num{1380}
and cells by \num{12}. The added markers name cell types that exist only
inside the added macro; no pre-existing cell's count changes. The series
\num{2316}, \num{9668}, \num{11048} across one, six and eight instances
is therefore consistent with the count being a property of the vendor
array and its instantiation, not of the surrounding design---but array
size and instance count move together across these three points, and a
sweep over the PDK's other macro sizes, which would separate them, has
not been run.
...
\paragraph{The scale series confounds two variables.} Array size and
instance count move together across the three points. A sweep over the
PDK's remaining macro sizes would separate them and has not been run.
