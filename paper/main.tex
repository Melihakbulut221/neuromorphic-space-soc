...
fixed here. \textbf{And the layout carries six macros where the design's
default builds eight}: the read-only memory's error-correcting code adds
two \num{512}$\times$\num{16} check macros, and every layout in this
repository is built with that code switched off, because the floorplan
can place six and not eight. So the decks above cover the random-access
memory's protection and not the read-only memory's, which has never been
placed, never seen a deck and never been through LVS. It is recorded in
the source as \emph{known and unplaced}; this is that sentence where a
reader of the deck results will meet it \\
\midrule
