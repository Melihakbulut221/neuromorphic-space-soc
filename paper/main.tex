...
% paper/claims.yaml and checked by paper/check_claims.py, which is
% configured to run in CI and has not been able to since 2026-09-03 --
% see Section 12. The
...
fails 3{,}088 setup endpoints at the slow corner and closes only at
\SI{25}{\nano\second}. Its geometric decks had been switched off for
three years' worth of runs on the strength of one experiment; we ran
them, and \textbf{of \num{39969214} Magic errors and \num{9668} KLayout
errors, zero fall outside the vendor SRAM macros}, while Netgen matches
...
boxes and KLayout \num{9668}, \textbf{all of both inside the six vendor
SRAM macros and none outside}; Netgen LVS \emph{circuits match uniquely}
...
with bit count} off the single-macro figure ($1.9985\times$ and
...
\emph{The count is a box count, not a violation count}, and the report
says so itself: its last line reads \emph{``should be divided by 3 or
4''}, so the distinct-violation figure is roughly 10.0 to 13.3 million.
The divisor changes the inside number and does not touch the outside
number, which is zero either way.

\emph{The zero is not the deck declining to look.} A positive control---
the same deck, same version, same options, on a top cell carrying
deliberate metal width and spacing violations---returns those
violations.
...
\emph{list} (355 and 193 names, checked against the vendor's own circuit
netlist and identical as sets), the instance count, and the net on all
\num{2130} macro pin connections---a swapped data bit or a supply
...
run returns none outside the macros, and all 123 are artefacts: every
rule involved requires metal wider than \SI{10}{\micro\meter}, and
...
\SI{25.21}{\nano\second}. The diagnosis that ranked them first had
...
\textbf{It did not close.} $-\SI{3.0306}{\nano\second}$ with 2{,}746
violating endpoints. What closes this design is the clock period, and
Section~\ref{sec:limits} says what that means for the frequency this
...
\paragraph{The checks are configured to run in CI and do not.}
Every workflow run on this repository since 3 September 2026 has been
\emph{refused before starting}, with the annotation \emph{``the job was
not started because recent account payments have failed or your spending
limit needs to be increased''}. That is nine runs of the licence check,
four of the claim checker and every documentation build: not nine
failures, nine non-starts. So every sentence in this paper and in the
repository that says a check ``runs in CI'' describes a configuration
and not an observed fact, and they have been corrected to say so. The
checks run locally, which is where every result reported here was
obtained; what is missing is the independent re-execution that CI is
for. Discovered on 10 September 2026 while auditing what was still
open---by looking, which is the only way this class of thing is ever
found.
