...
%
% Every number printed in this paper is registered in paper/claims.yaml
% and re-derived by paper/check_claims.py, which runs in CI. A claim that
% cannot be re-derived is recorded there as read-by-hand from a named
% artefact, and the count of those is printed in Section 10 rather than
% left implicit.
...
measurement on RTL and on placed netlists, the system-on-chip layout
fails 3{,}088 setup endpoints at the slow corner, and no Magic DRC,
KLayout DRC or Netgen LVS result exists for it---the decks were switched
off on the strength of a single-macro experiment in which the vendor SRAM
macro produced \num{1106478} Magic errors, \emph{every one of them inside
the macro footprint and none outside}, and we report below what happens
when they are switched back on.
...
declared replicas into one physical bank, and the register-transfer-level
...
technology mappers.
In the die it is false again, and this is measured here for the first
time: the three replica banks share one region of the layout, the minimum
cross-replica cell separation is \SI{3.78}{\micro\meter} centre-to-centre
and \SI{0.00}{\micro\meter} edge-to-edge---the cells touch---49 pairs
abut, and for 103 of 165 configuration flip-flops the nearest other
configuration flip-flop belongs to a \emph{different} replica.
...
identically and become one. The netlist that fed a hardening run contains
362 references to \texttt{cfg\_a}, none to \texttt{cfg\_b} and none to
\texttt{cfg\_c}. The RTL declares \num{1161} flip-flops; the netlist has
\num{1045}.
...
Every figure is re-derived by \file{paper/check\_claims.py} or is recorded
there as read once from the named artefact.}
\label{tab:inventory}
\small
...
Geometric sign-off & \textbf{none as of the run this paper measures}. The
decks are switched off in the configuration and were never run on this
design \\
Why & a single-macro experiment: one \texttt{RM\_IHPSG13} returns
\num{1106478} Magic DRC error boxes \emph{of which none falls outside the
macro footprint}, \num{2316} KLayout errors all naming cells inside the
macro hierarchy, and 359 Netgen LVS errors that are a \emph{name and bus
delimiter} mismatch rather than a geometric one. An upstream integration
failure, not a defect of this design---and see \S\ref{sec:decks} for what
happens when the decks are pointed at this design with the vendor macro
abstracted \\
\midrule
\multicolumn{2}{l}{\textbf{What does not exist}}\\
\midrule
Silicon & none, no engineering sample, no schedule for one \\
Radiation data & none of any kind. Neither SkyWater 130\,nm nor IHP
SG13G2 publishes TID, SEE or SER characterisation \\
Application accuracy & no classification result for this design on any
task, and none has ever existed \\
A clock frequency for the SoC & withheld. \SI{50}{\mega\hertz} is not
achieved and the measured non-closure is not a specification. The
\emph{pilot's} figure is published above because it signs off on every
deck an open flow can run \\
\bottomrule
\end{tabularx}
\end{table}
...
internals the vendor certifies. It is not deck relaxation---it changes
what is looked at, not what counts as an error---and the diagnosis above
is precisely the evidence that it would work, because a deck that never
enters the macro cannot report the errors that are all inside it.
...
Three replicas need six distinct functions, and one of the three is the
identity, so \textbf{three bits is the first width at which a third
replica has a function left to take}. Below it, structural hashing
collapses one replica whatever the source says.

This is a statement about defeating a specific optimisation over a
specific family of transforms, not an information-theoretic bound, and it
should be read that way. It is nonetheless a design rule with immediate
...
\emph{bundled} into 22-, 23-, 25- and 29-bit words before triplication,
because the bundle has functions to spare and the flags do not.
...
hardening. The project records four instances where this was caught after
the fact, of \SI{683.1216}{\micro\meter\squared},
\SI{58.17}{\micro\meter\squared}, \SI{27.216}{\micro\meter\squared} and
\SI{25.4394}{\micro\meter\squared}, and \textbf{every one of them was in
the direction that understates the cost}.
...
By mode, \textbf{51 are \texttt{prove}} by $k$-induction---the only
unbounded results anywhere in the project---36 are bounded model
checking and 31 are cover, with depths from 1 to 220. All 118 have a
...
all 336 such copies against their tracked originals: raw comparison says
336 differ, and the entire difference is a three-line licence header
added later. Stripping only those headers gives \textbf{336 identical, no
real difference}, so the verdicts do correspond to the design as it
...
Across 25 property files there are 106 \texttt{assume} statements, 6
symbolic constants, and \textbf{zero} \texttt{restrict}. Six property
files carry \emph{no} assumptions at all---the SEC-DED codec, the
byte-lane codec, the register-file codec, the memory ECC integration, the
LIF memory codec and the TMR voter---and these are the strongest results
in the project. The SEC-DED property file states each property as an
implication on Hamming weight rather than assuming one.

The load-bearing assumption is stated plainly because it bounds
everything: \textbf{the single-fault assumption appears in 14 of the 25
property files.} Every fault-tolerance proof in this work is scoped to at
most one bad replica and at most one flipped bit, by construction. It
follows that no proof here can express a multiple-bit upset, and
Section~\ref{sec:placement} is precisely about a geometry in which
multiple-bit upsets are the relevant question.
...
\texttt{liveness} fail in both, and the six multiply and unsigned-divide
checks stop in both. That identity is the substance of the claim that the
substitution changed nothing, and it holds.
...
70 base instructions have no verdict in either run. Bus errors are
...
This bench passed its own per-deposit verification on all 2{,}800 runs
of that campaign---each injection is checked to have flipped the bit it
...
reported 2, 1 and 1 differing flip-flops out of 964 on the three records
it passed; after, it reports \textbf{0 of 964 over 18{,}681 cycles on all
four}.
...
\textbf{1{,}158 classify identically and 17 disagree}. Every disagreement
class was characterised rather than reported as noise: the synthesiser
storing two register-transfer registers in one flip-flop each, so the
netlist is strictly worse on a subset; state-machine re-encoding
producing netlist flip-flops with no register-transfer twin, which are
the most lethal storage in the design per bit; and one class where the
netlist is better.
...
\textsc{[Results pending: the three decks are running over this design's
final mask data---Magic DRC classified by whether each error falls inside
one of the six macro bounding boxes, KLayout DRC classified by the cell
each violation names, and Netgen LVS with the macro declared a black box
so that the connectivity of everything else is compared. The claim this
section will make is bounded in advance: at most ``no design-rule error
in the geometry this project drew, with the vendor macro abstracted and
that exclusion named''---never ``the design is DRC-clean''.]}
...
\paragraph{What is being measured.}
\textsc{[Results pending: which corner the resizer optimises is being
verified from the flow's own step classes rather than inferred from the
key names; the 3{,}088 endpoints are being histogrammed by slack to
establish whether this is a cliff or a plateau, which decides whether a
configuration change can close it at all; and a re-hardening with the
repair bound to the corner it is checked at, with timing-driven
placement, is running into a new directory. Whatever it returns will be
reported against the attribution rule of Section~\ref{sec:determinism}:
an improvement is not credited to a change unless it can be separated
from the netlist landing differently.]}
...
components. Every row is re-derived by \file{paper/check\_claims.py}.}
\label{tab:placement}
\small
\begin{tabular}{lr}
\toprule
...
Cross-replica pairs & \num{9075} \\
Centre-to-centre, minimum & \SI{3.78}{\micro\meter} (one row) \\
Centre-to-centre, median & \SI{113.47}{\micro\meter} \\
\textbf{Edge-to-edge, minimum} & \textbf{\SI{0.00}{\micro\meter}} \\
Pairs whose cell outlines touch & \textbf{49} \\
...
\textbf{Nearest fellow configuration flip-flop is in another replica} &
\textbf{103 of 165} \\
\bottomrule
\end{tabular}
\end{table}
...
The row that needs no threshold and no interpretation is the last one.
For \textbf{103 of the 165 configuration flip-flops, the nearest other
configuration flip-flop belongs to a different replica.} Three separated
banks give the opposite.
...
One consequence was not designed and is worth recording. Replica C is
the best-separated bank, and the reason is the mixing transform of
Section~\ref{sec:mechanisms}, which was chosen for a purely logical
reason---to stop structural hashing merging it. \textbf{A defence chosen
against the synthesiser bought physical margin by accident}, for one of
the three pairings, when a vote needs only two replicas corrupted.
...
30 of the 55 bits; the committed tool reports the conservative figure,
because a lower bound quoted as a measurement is the failure this paper
is about.
...
row. Two designs, four layouts, three attribution methods, one answer.
...
What it does establish, and what we did change, is the wording of every
claim this project makes about the state in question. \textbf{``Protected
by triple modular redundancy'' is false as stated and now reads
``triplicated in the netlist'' throughout.} That is a correctness change,
...
worst negative slack and moved the problem rather than solving it,
...
Netlist: 362 references to \texttt{cfg\_a}, \textbf{0} to
...
\section{Flow determinism, and why every delta in this paper is
attributable}
\label{sec:determinism}

Every cost figure here is a difference between two runs of the same flow.
That difference means nothing unless the flow is deterministic, so we
measured it.
...
The single-fault assumption appears in 14 of 25 property files. No proof
here can express the failure mode that Section~\ref{sec:placement}'s
geometry makes relevant.
...
an instrument that lives at the netlist. In the die it is still not
protected, and the instrument we built cannot see that, because it runs
before the die exists.
...
\begin{verbatim}
# every number this paper prints
python3 paper/check_claims.py

# the placement measurement of Section 8
...
# the netlist censuses of Section 9
