...
% paper/claims.yaml and checked by paper/check_claims.py in CI. The
% registry is NOT a census of every number printed here -- the body
% prints about 174 numeric literals and the registry holds 27 -- and the
% checker's own summary says how many of its claims are re-derived by a
% command and how many are read once by hand from a named artefact.
...
the macro footprint and none outside}; \S\ref{sec:decks} states the scope
they were switched off at, and no result from re-running them is reported
in this version.
...
by no test: these three are the only figures in this section a reader
cannot re-derive, and they are printed to bound the direction of the
undercount and for nothing else.} The committed tool reports the
