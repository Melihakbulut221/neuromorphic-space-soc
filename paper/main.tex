...
\num{9668} KLayout \emph{markers} on that layout, zero fall outside the
vendor SRAM macros}. Neither headline is a count of distinct violations and we
...
touches the outside number, which is zero throughout. \emph{And ``inside
a macro'' is not the same as ``the vendor's'': of the \num{39969214},
\num{128} are not produced by the macro checked on its own, and
\S\ref{sec:decks} says what makes them.} Netgen matches the layout to
the netlist uniquely with those macros abstracted.
\emph{Superseded 14 September 2026, and left standing: every deck figure
above is the six-macro layout's. The eight-macro layout has since been
through the same decks. KLayout returns \num{11048} markers and still
\textbf{zero outside}: the \num{1380} added markers all name four cell
types that exist only inside the newly added macro, not one pre-existing
cell's count moves, and \num{4464} of the \num{11048} are the doubled
population, leaving \num{6584} distinct. Netgen still matches uniquely.
The seventeen-hour full-hierarchy Magic run has not been repeated, so
neither the \num{39969214} nor the \num{128} has an eight-macro
counterpart, and the newly added macro has never been checked on its own
either. What the cheap abstracted deck does give is stronger than the
case \S\ref{sec:decks} argues box by box: of its \num{170} boxes,
\num{20} sit inside a macro footprint and \textbf{every one of the other
\num{150} lies within \SI{0.500}{\micro\meter} of a macro edge}.}
...
before the repair. \emph{An earlier draft put the repair's cost at
``five flip-flops and about twenty-three cells''; measured on 12
September 2026, both numbers are wrong.} Synthesised either side of the
repair with one recipe and nothing else changed, the system-on-chip goes
from \num{46116} cells and \num{6087} flip-flops to \num{45426} and
\num{6090}: \textbf{three flip-flops more and six hundred and ninety
cells fewer}, and \SI{353.58}{\micro\meter\squared} smaller. Five
registers were written into the sources and two do not survive mapping.
The cell count moves in the opposite direction from the one asserted and
by thirty times the magnitude, because at this scale a cell delta
measures the technology mapper's re-synthesis of the thousand cells
around a change and not the change---which is the finding this project
already recorded once, and then published a cell count anyway. The
flip-flop delta is the one that is recipe-invariant, and it is three. \emph{Those numbers remain true of those runs}, which
...
reproduce the figures below, and the difference is three flip-flops.
...
fixed here. \textbf{And the layout these decks ran on carries six macros where the
design's default builds eight}: the read-only memory's error-correcting
code adds two \num{512}$\times$\num{16} check macros, and the layout
above is built with that code switched off. So the deck results in this
row cover the random-access memory's protection and not the read-only
memory's. \textbf{Corrected \mbox{2026-09-13}:} the reason given here
was wrong. It said the floorplan \emph{can place six and not eight};
it can place eight, and the obstacle was an estimate taken along the
wrong axis --- the space for a check macro is not beside the read-only
memory but in the \SI{290.24}{\micro\meter} of unused height its
shorter macro leaves in its own row. A layout with all eight was built
and routed that day, with zero detailed-routing violations, and two
decks were run on it: Netgen LVS reports \emph{circuits match
uniquely}, and the KLayout deck puts \num{0} of its \num{11048}
markers --- \num{6584} distinct shapes, the two Schottky rules
reporting one population twice --- outside the vendor macro
hierarchy. All four decks
have since been run on it: Netgen LVS reports \emph{circuits match
uniquely}; the KLayout deck puts \num{0} of its \num{11048} markers
--- \num{6584} distinct shapes, the two Schottky rules reporting one
population twice --- outside the vendor macro hierarchy; and Magic
reports \num{182} boxes of which \num{20} fall inside a macro
footprint and every one of the remaining \num{162} lies within
\SI{0.5}{\micro\meter} of a macro edge, none in open routing area.
That layout is \textbf{not} the layout these deck numbers describe,
and no timing, power or fault-injection result in this paper has been
re-measured on it \\
\midrule
...
\emph{On the eight-macro layout the same experiment returns a stronger
form of that conclusion, and this time it does not need the mechanism to
state it.} The abstracted run over \texttt{s83ant} returns 182 boxes, 20
inside a macro footprint and 162 outside---and \textbf{every one of the
162 lies within \SI{0.500}{\micro\meter} of a macro footprint edge},
measured as the gap from each box to the nearest of the eight
rectangles the placement reserves. The gaps take six values and no
others: \SI{0.220}{\micro\meter} on 31 boxes,
\SI{0.260}{\micro\meter} on 55, \SI{0.280}{\micro\meter} on 5,
\SI{0.320}{\micro\meter} on 54, \SI{0.380}{\micro\meter} on 12 and
\SI{0.500}{\micro\meter} on 3, with two boxes touching an edge exactly.
Not one sits out in the field of standard cells and routing where almost
every polygon in this layout is. That is this paragraph's claim in its
plainest form---the abstraction produces errors at the boundary it
invented and nowhere else---and unlike the wide-metal argument it is a
distance rather than an interpretation. The pre-repair layout
\texttt{s83romecc5} gives 170 boxes, the same 20 inside, 150 outside and
the same \SI{0.500}{\micro\meter} maximum, so the bound is not an
accident of one run. \emph{And the test applies backwards}: run over
\texttt{s77gate}'s 107 outside boxes it returns a maximum of
\SI{0.380}{\micro\meter}. The property was true of the six-macro
measurement all along and nobody had measured it, which is the ordinary
way a strong result gets missed---by not asking the instrument a
...
extent. The full run over the real geometry reports nothing at that
coordinate, so it too is present only in the abstracted view.

\emph{The mechanism was unknown when this paragraph was written and is
now established, 12 September 2026, by reproduction and two knockouts.}
The marker is neither a sliver of metal nor a gap: it is the
\SI{0.020}{\micro\meter} \emph{shortfall} of a spacing check. A
single routed Metal4 wire has its left edge at $x =
\SI{904.700}{\micro\meter}$; \SI{0.220}{\micro\meter} to its left,
at $x = \SI{904.480}{\micro\meter}$, is the right edge of a
\SI{4.0}{\micro\meter}-wide Metal4 \emph{obstruction} that the
macro's boundary description draws out to the cell edge. The tool paints
that obstruction as its own layer type and the deck's wide-metal rule
names that type explicitly, so the obstruction is treated as
\SI{4.0}{\micro\meter} of metal related to no net, and the rule asks
for \SI{0.240}{\micro\meter}. The router had obeyed the
\SI{0.21}{\micro\meter} the macro's own description states for that
obstruction---which the abstraction drops, because the tool has no
per-shape spacing---so the wire stands \SI{0.020}{\micro\meter} short
of what sign-off asks. In the real geometry the macro's nearest Metal4
stops \SI{4.480}{\micro\meter} away: the obstruction covers
\SI{4.26}{\micro\meter} of empty silicon.

So the last box has the same shape as the other 122 after all---a
boundary blanket quarrelling with a real route---and it took a different
rule to get there. Removing the macro instance from a clipped copy makes
it vanish; so does deleting that one obstruction rectangle and nothing
else. \textbf{We did not move the route.} Nothing is within
\SI{4.480}{\micro\meter} of it in the geometry that would be
fabricated, and shifting a wire \SI{0.020}{\micro\meter} to satisfy a
check that is itself an artefact would be optimising for the
instrument.
...
\paragraph{And the full run's own headline is narrower than it reads,
by \num{128} boxes.} ``Every error box falls inside a vendor macro
footprint'' is a containment test on four coordinates. It carries no
layer, no net and no cell of origin, so it cannot distinguish a
violation the macro arrives with from one that appears when the macro is
merged into a larger layout---and that distinction is not hypothetical
here. Checking each macro \emph{on its own} and mapping every box back
into macro-local coordinates, the two read-only-memory instances each
carry \num{64} boxes, \num{128} in all, that the macro alone does not
produce. They fire on a rule about abutment \emph{between subcells}.
The merged layout's flattened geometry for that macro is identical to
the vendor's on all 38 layers; what differs is its hierarchy, 152 called
cells against 140, because the merge re-expresses the macro's via cells
under different names. The larger macro, whose child count is unchanged,
contributes none. We could not attribute them to this design's routing:
no routed shape sits in the window where they repeat.

So the outside number stays zero and stays checkable, and the inside
number stops meaning ``all the vendor's''. \num{128} of nearly forty
million is not a reason to doubt the layout---the geometry that would be
fabricated is unchanged---but a containment test that was read as a
statement about blame was, for those \num{128}, answering a different
question from the one being asked of it.

All of that is \texttt{s77gate}'s full run, and none of it has been
re-asked of the eight-macro layout, because there is no full run there
to ask. The two \texttt{1024x32} instances that carried the \num{128}
are still in the floorplan and the merge that re-expressed their via
cells is the same merge, so we expect the \num{128} to recur and two
more macros to be added to the question. That is a prediction. It is not
a measurement, and this paragraph will not be read as though it were
one.
...
25-bit accelerator bank---on two independent hardening runs,
\texttt{s71boot} and \texttt{s75w9}, by a
