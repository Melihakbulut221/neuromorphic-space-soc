...
CELL_AREA = 396177.6420   # um2, hw/soc/flow/syn_soc_top.sh, SOC_MEM=sram
HALO = 10.0
...
def build_islands(channel, pdk_root):
...
def build(channel, pdk_root, density_pct=None):
    ram_w, ram_h = lef_size(pdk_root, RAM)
    rom_w, rom_h = lef_size(pdk_root, ROM)
...
def main():
    ...
    fp = (build_islands(channel, pdk_root) if a.arrangement == "islands"
          else build(channel, pdk_root))
    ...
    print(f"cells          {CELL_AREA:10.2f} um2  "
